...
func (p *Parser) ProcessLeafRef(e *yang.Entry, resfullPath string, activeResPath *config.Path) (*config.Path, *config.Path, bool) {
	...
	default:
		...
		case "leafref":
			...
			p.AppendElemInConfigPath(remotePath, elem, k)
			...
			if strings.Contains(*p.ConfigGnmiPathToXPath(remotePath, false), *p.ConfigGnmiPathToXPath(activeResPath, false)) {
				...
				if *p.ConfigGnmiPathToXPath(remotePath, false) != *p.ConfigGnmiPathToXPath(activeResPath, false) {
					...
					localPath = p.TransformConfigPathAsRelative2Resource(localPath, activeResPath)
					remotePath = p.TransformConfigPathAsRelative2Resource(remotePath, activeResPath)
...
func (p *Parser) ValidateLeafRef(kind LeafRefValidationKind, x1, x2 interface{}, definedLeafRefs []*LeafRef, log logging.Logger) (bool, []*ResolvedLeafRef, error) {
	...
	for _, leafRef := range definedLeafRefs {
		...
		resolvedLeafRef := &ResolvedLeafRef{
			LocalPath:  p.DeepCopyConfigPath(leafRef.LocalPath), // used for path
			RemotePath: p.DeepCopyConfigPath(leafRef.RemotePath),
			Value:      "",
			Resolved:   false,
		}
		...
		tc := &TraceCtxt{
			Path:             p.DeepCopyConfigPath(leafRef.LocalPath), // used to walk through the object -> this data will not be filled in
			Idx:              0,
			Msg:              make([]string, 0),
		...
		*/
		...
		for _, resolvedLeafRef := range tc.ResolvedLeafRefs {
			...
			if resolvedLeafRef.Resolved {
				...
				tc := &TraceCtxt{
					Path:   p.DeepCopyConfigPath(resolvedLeafRef.RemotePath), // used to walk through the object -> this data will not be filled in
					Idx:    0,
					Msg:    make([]string, 0),
